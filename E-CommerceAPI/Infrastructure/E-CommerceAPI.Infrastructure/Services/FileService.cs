...
namespace E_CommerceAPI.Infrastructure.Services
{
    public class FileService : IFileService
    {
        ...
        public async Task<bool> CopyFileAsync(string path, IFormFile file)
        {
            try
            {
             ...
             FileShare.None, 1024 * 1024, useAsync: false);
                ...
                await fileStream.FlushAsync(); //todo // fileStream ile elaqeli olaraq  temizlemek isi gorur !
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<(string fileName, string path)>> UploadAsync(string path, IFormFileCollection files)
        {
            ...
            List<(string fileName, string path)> datas = new();  //(string fileName, string path) Tupple dir
